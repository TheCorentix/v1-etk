      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-white h-full flex flex-col shadow-2xl border-l border-[#ECECEC] z-10"
          >
            {/* Header */}
            <div className="p-6 border-b border-[#ECECEC] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-serif text-lg tracking-wider text-text-custom">YOUR SHOPPING BAG</span>
              </div>
              <button
                onClick={onClose}
                className="p-2 text-[#181818] hover:text-[#B68D40] focus:outline-none"
            ...
            {checkoutSuccess ? (
              ...
              <div className="flex-grow flex flex-col items-center justify-center p-8 text-center space-y-6">
                ...
                <h3 className="font-serif text-2xl text-[#181818] tracking-wider">ORDER CONFIRMED</h3>
                ...
                <span className="font-mono text-sm font-semibold text-[#B68D40] tracking-widest bg-[#F8F6F2] px-4 py-2 border border-[#D9C7A3]">
                  {orderNumber}
                </span>
              ...
              <div className="flex-grow flex flex-col items-center justify-center p-8 text-center space-y-6">
                ...
                <h3 className="font-serif text-lg tracking-wider text-text-custom">YOUR BAG IS EMPTY</h3>
              ...
              <>
                ...
                <div className="flex-grow overflow-y-auto p-6 space-y-6 custom-scrollbar">
                  {cart.map((item) => (
                    <div key={`${item.id}-${item.size}`} className="flex gap-4 pb-6 border-b border-[#ECECEC] items-start">
                      ...
                      <div className="flex-grow space-y-1">
                        ...
                        <h4 className="font-serif text-xs text-[#181818] uppercase tracking-wider leading-tight">
                          {item.name}
                        </h4>
                        <p className="text-[10px] text-neutral-500 font-sans">
                          Size: <span className="font-medium text-[#181818]">{item.size}</span>
                        </p>
                        
                        {/* Quantity Controls */}
                        <div className="flex items-center gap-3 pt-2">
                          <div className="flex items-center border border-[#ECECEC] bg-[#F8F6F2]">
                            <button
                            ...
                            <span className="px-2 text-xs font-sans font-medium text-[#181818]">{item.qty}</span>
                            <button
                              onClick={() => updateQuantity(item.id, item.size, item.qty + 1)}
                              className="px-2 py-1 text-xs hover:text-[#B68D40] focus:outline-none"
                      ...
                      <div className="text-right shrink-0">
                        <span className="font-sans text-xs font-medium text-[#181818] block">
                  ...
                  <div className="bg-[#F8F6F2] p-4 border border-[#D9C7A3] flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <Gift className="w-5 h-5 text-[#B68D40]" />
                      <div>
                        <h5 className="text-[11px] font-sans font-semibold tracking-wider text-[#181818] uppercase">
                  ...
                  <form onSubmit={handleApplyCoupon} className="space-y-2">
                    <div className="flex gap-2">
                      <input
                        ...
                        disabled={!!coupon}
                        className="flex-grow bg-transparent border border-neutral-300 px-3 py-2 text-xs font-sans tracking-widest text-[#181818] placeholder-neutral-400 focus:outline-none focus:border-[#B68D40] disabled:bg-neutral-100 disabled:"
                      />
                      {coupon ? (
                        <button
                          type="button"
                        ...
                        <button
                          type="submit"
                          disabled={isApplying}
                          className="px-4 py-2 border border-[#181818] text-[#181818] hover:bg-[#181818] hover:text-white text-[10px] uppercase tracking-widest transition-colors font-sans disabled:opacity-50"
                        >
                          Apply
                        </button>
                      )}
                ...
                <div className="p-6 border-t border-[#ECECEC] bg-white space-y-4">
                  <div className="space-y-1.5 text-xs text-neutral-500 font-sans uppercase tracking-wider">
                    ...
                    <div className={`flex justify-between text-[10px] font-semibold ${shipEstimate.inr === 0 ? 'text-[#3E7C59]' : 'text-neutral-600'}`}>
                  ...
                  <div className="border-t border-neutral-100 pt-3 flex justify-between items-end">
                    <span className="font-serif text-sm tracking-wider text-[#181818]">TOTAL ESTIMATED</span>
